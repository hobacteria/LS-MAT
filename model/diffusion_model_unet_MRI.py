...
class DiffusionModelUNetMRI(nn.Module):
    """
    ...
    def __init__(
        ...
        if cfg:
            dropout_space = 1
        else : dropout_space = 0
        new_time_embed_dim = time_embed_dim
        if self.include_sub_cat_embed:
            self.sub_cat_embed_layer = self._create_embedding_module(3 + dropout_space, time_embed_dim)
            new_time_embed_dim += time_embed_dim
        if self.include_sex_embed:
            self.sex_embed_layer = self._create_embedding_module(2 + dropout_space, time_embed_dim)
            new_time_embed_dim += time_embed_dim
    ...
    def _get_input_embeddings(self, emb, sub_cat_index, sex_index,modality_index,age_index, spacing):
        if self.include_sub_cat_embed:
    ...
    def forward(
        ...
        emb = self._get_input_embeddings(emb, sub_cat_index_tensor, sex_index_tensor, modality_index_tensor,age_tensor,spacing_tensor)
        h = self.conv_in(x)
